...
#if 0
typePMemReturn PersistentMemory_Check(void){
  u8 index;
  u8 ret;
  ...
  /* FIXME: the next line contains a cast to (u16 *) to silence compiler -
   * strictly speaking, we should change the function definition to accept a
   * const u16 * since this data would not be changed. This should apply to all
   * other functions. */
  WriteI2CBytes(MB_I2C_BUS_ID, MAX31785_I2C_DEVICE_ADDRESS, (u16 *) WriteBytes, 2);
  if (ret != XST_SUCCESS){
    return PMEM_RETURN_ERROR;
  }

  PMBusReadI2CBytes(MB_I2C_BUS_ID, MAX31785_I2C_DEVICE_ADDRESS,
      MFR_LOCATION_CMD, ReadBytes, MFR_LOCATION_LEN);
  if (ret != XST_SUCCESS){
    return PMEM_RETURN_ERROR;
  }
  ...
  for (index = 0; index < MFR_LOCATION_LEN; index++){
    debug_printf("%x ", ReadBytes[index]);
  }
  debug_printf("\r\n");
...
/*
 * Write data to the relevant byte or all of the bytes in the persistent
 * memory register.
 * This function accepts enumerated typePMemByteIndex values.
 */

typePMemReturn PersistentMemory_WriteByte(typePMemByteIndex byte_index, u8 byte_data){
  u8 ret;
  u8 index;
  const u16 WriteBytes1[2] = {PAGE_CMD, 255};
  u16 WriteBytes2[MFR_LOCATION_LEN + 1] = {0};
  u16 ReadBytes[8] = {0x0};

  Xil_AssertNonvoid((HMC_RECONFIG_COUNT_BYTE == byte_index) ||
      (DHCP_RECONFIG_COUNT_BYTE == byte_index));
  ...
  if (ALL_BYTES == byte_index){
    for (index = 0; index < MFR_LOCATION_LEN; index++){ 
      WriteBytes2[index + 1] = byte_data;
    }
  } else {
    ret = PMBusReadI2CBytes(MB_I2C_BUS_ID, MAX31785_I2C_DEVICE_ADDRESS,
        MFR_LOCATION_CMD, ReadBytes, MFR_LOCATION_LEN);
    ...
    for (index = 0; index < MFR_LOCATION_LEN; index++){ 
      WriteBytes2[index + 1] = ReadBytes[index];
    }
    WriteBytes2[byte_index + 1] = byte_data; /* set the data of relevant byte */
  }

  WriteBytes2[0] = MFR_LOCATION_CMD;
  ret = WriteI2CBytes(MB_I2C_BUS_ID, MAX31785_I2C_DEVICE_ADDRESS, WriteBytes2, 9);
  if (ret != XST_SUCCESS){
    return PMEM_RETURN_ERROR;
  }
  ...
  return PMEM_RETURN_OK;
}

typePMemReturn PersistentMemory_ReadByte(typePMemByteIndex byte_index, u8 *read_byte_data){
  u8 index;
  u8 ret;
  const u16 WriteBytes[2] = {PAGE_CMD, 255};
  const u16 MFR_LOCATION_default_values[8] = {0x30, 0x31, 0x30, 0x31, 0x30,
  ...
  *read_byte_data = ReadBytes[byte_index];
