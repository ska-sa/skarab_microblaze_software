...
void init_wdt(XWdtTb *WatchdogTimer){
  ...
  if (WatchdogTimerConfig == NULL){
    log_printf(LOG_SELECT_GENERAL, LOG_LEVEL_ERROR, "WDT  [..] failed to find wdt config\r\n");
    return;
  }
  ...
  if (ret == XST_DEVICE_IS_STARTED){
    log_printf(LOG_SELECT_GENERAL, LOG_LEVEL_ERROR, "WDT  [..] failed to initialize the wdt since it is already running\r\n");
    ...
    if (ret != XST_SUCCESS){
      log_printf(LOG_SELECT_GENERAL, LOG_LEVEL_ERROR, "WDT  [..] self-test failed\r\n");
    } else {
      /* finally, start the watchdog. The timebase automatically reset*/
      XWdtTb_Start(WatchdogTimer);
      log_printf(LOG_SELECT_GENERAL, LOG_LEVEL_INFO, "WDT  [..] starting the watchdog timer...\r\n");
    }
  }
