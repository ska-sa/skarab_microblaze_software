...
u8 get_skarab_serial(u8 *sk_buffer, u8 sk_length){
  ...
  if (skarab_serial[0] == 0xff){
    ...
    if (ret == XST_SUCCESS){
      ...
      if (uRomCRC != uRom[7]){
        error_printf("MB 1-wire CRC incorrect.\r\n");
        ...
        if (ret == XST_FAILURE){
          error_printf("Failed to read skarab serial number.\r\n");
          skarab_serial[0] = 0xFF;
          skarab_serial[1] = 0xFF;
          skarab_serial[2] = 0xFF;
          skarab_serial[3] = 0xFF;
    ...
    if (skarab_serial[0] != 0x50){    /* CHECK FOR 'P' */
      error_printf("Trying again to read serial number.\r\n");
  ...
  if (peralex_serial[0] == 0xff){
    ...
    if (ret == XST_SUCCESS){
      ...
      if (uRomCRC != uRom[7]){
        ...
        if (ret == XST_FAILURE){
          error_printf("Failed to read motherboard serial number.\r\n");
          peralex_serial[0] = 0xFF;
          peralex_serial[1] = 0xFF;
          peralex_serial[2] = 0xFF;
        }
